...
function meanDistance(dx: number, dy: number, x: number, y: number): number {
...
const images: number[] = new Array(4 + Math.ceil(Math.random() * 2)).fill(NaN);
...
for (const image of images) {
  ...
  if (added.length !== 0) {
    while (
      meanDistance(dx, dy, ...(added[findClosestImage(dx, dy, added)].points)) <
        144
    ) {
      if (Math.random() < 0.5) {
        dx += 36;
